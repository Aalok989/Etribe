            ...
            {!editMode ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-8 mb-8 w-full">
                {Array.from({ length: 10 }).map((_, i) => (
                  <div key={i}>
                    <span className="font-bold text-gray-700 block mb-1">Additional Field {i + 1}</span>
                    <span className="text-gray-900 text-base font-normal">{data[`additionalField${i + 1}`]}</span>
                  </div>
                ))}
              </div>
            ) : (
              <form className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-8 mb-8 w-full" onSubmit={handleSubmit}>
                {Array.from({ length: 10 }).map((_, i) => (
                  <div className="flex flex-col gap-1" key={i}>
                    <label className="font-medium text-gray-700">Additional Field {i + 1}</label>
                    <input
                      type="text"
                      name={`additionalField${i + 1}`}
                      value={form[`additionalField${i + 1}`]}
